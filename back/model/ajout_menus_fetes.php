<?php 
    ...
    function ajoutPictureBDD($name, $pictureCh) {
        ...
        $req->execute();
        $pictureID = $req->fetch();
        ajoutMenusFetes($name, $pictureID[0]);

    }

    function ajoutMenusFetes($name, $pictureID) {

        $query = "INSERT INTO menus_fetes (name, id_picture)
        ...
        if($req->execute() > 0) {
            $success = "un menus de fetes à étais ajouter";
            return $success;
        }
    }
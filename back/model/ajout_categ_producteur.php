<?php
    ...
    function ajoutPictureBDD($name, $pictureCh) {
        ...
        $req->execute();
        $pictureID = $req->fetch();
        ajoutCategProducteur($name, $pictureID[0]);
    }

    function ajoutCategProducteur($name, $pictureID) {
        $name = htmlspecialchars($_POST['name']);

        $query = "INSERT INTO categ_producteur (name, id_picture) VALUES (:name, :id_picture)";
        $req = dbConnect()->prepare($query);
        $req->bindValue(':name', $name, PDO::PARAM_STR);

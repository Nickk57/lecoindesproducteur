<?php
    ...
    function selectCategory($id) {
        ...
        $query = "SELECT * FROM categ_producteur WHERE id = :id";
        $req = dbConnect()->prepare($query);
        $req->bindValue(':id', $id, PDO::PARAM_INT);
        $req->execute();
        $selCategProd = $req->fetch(PDO::FETCH_ASSOC);
        return $selCategProd;
    }
    ...
    function ajoutPictureBDD($id, $name, $pictureCh) {
        ...
        $req->execute();
        $pictureID = $req->fetch();
        ajoutCategory($id, $name, $pictureID[0]);
    }

    function modificCateg($id, $name, $pictureID) {

        $query = "UPDATE categ_producteur SET name = :name, id_picture = :id_picture 
        WHERE id = :id";
        $req = dbConnect()->prepare($query);
        $req->bindValue(':id', $id, PDO::PARAM_INT);
        $req->bindValue(':name', $name, PDO::PARAM_STR);
